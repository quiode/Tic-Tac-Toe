...
function start_game() {
    ...
    // makes the grid
    var box_width = default_settings.canvas_dimensions.x / default_settings.grid_dimensions.x;
    var box_height = default_settings.canvas_dimensions.y / default_settings.grid_dimensions.y;
    for (var index_1 = 0; index_1 < default_settings.grid_dimensions.x; index_1++) {
        filled_boxes.push([]);
        for (var index_2 = 0; index_2 < default_settings.grid_dimensions.y; index_2++) {
            context.beginPath();
 ...
 */
function circle(radius, center, fill_color, stroke_color) {
    if (fill_color === void 0) { fill_color = "black"; }
    if (stroke_color === void 0) { stroke_color = "white"; }
    // gets the html elements
    var canvas = document.getElementById("canvas");
    var context = canvas.getContext("2d");
    // makes the circle
 ...
 * @param stroke_color the color for the border of the circle
 */
function square(side, center, fill_color, stroke_color) {
    if (fill_color === void 0) { fill_color = "black"; }
    if (stroke_color === void 0) { stroke_color = "white"; }
    // gets the html elements
    var canvas = document.getElementById("canvas");
...
function canvas_clicked(event) {
    ...
    if (click_in_box.x < 0 || click_in_box.y < 0) {
        ...
        var box = { row: null, column: null };
        // gets the row
        box.column = Math.floor(click_in_box.x /
            (default_settings.canvas_dimensions.x / default_settings.grid_dimensions.x));
        // gets the column
        box.row = Math.floor(click_in_box.y /
        ...
        if (typeof filled_boxes[box.column][box.row] != "undefined") {
            ...
            if (player_turn) {
                ...
                square(box_width * 0.75, {
                    x: box.column * box_width + box_width / 2,
                    y: box.row * box_height + box_height / 2
                }, default_settings.player_1.colors.primary_color, default_settings.player_1.colors.secondary_color);
            }
            else {
                circle((box_width / 2) * (1 - 0.25 * 2), {
                    x: box.column * box_width + box_width / 2,
                    y: box.row * box_height + box_height / 2
                }, default_settings.player_2.colors.primary_color, default_settings.player_2.colors.secondary_color);
            }
            // changes the player
 ...
 */
function calculate_click(x, y) {
    // gets the html elements
    var canvas = document.getElementById("canvas");
    // calculates x-coordinate in the box
    var in_box_x = x - (window.innerWidth - default_settings.canvas_dimensions.x) / 2;
    var in_box_y = y - (window.innerHeight - default_settings.canvas_dimensions.y) / 2;
...
function win_checker() {
    ...
    var box_height = default_settings.canvas_dimensions.y / default_settings.grid_dimensions.y;
    // checks all columns
    filled_boxes.forEach(function (column) {
        for (var big_index = 0; big_index < column.length - default_settings.game_settings.ponts_to_win + 1; big_index++) {
            var temp_array = [];
            for (var index = big_index; index < default_settings.game_settings.ponts_to_win + big_index; index++) {
                temp_array.push(column[index]);
            }
            // checks if all elements are the same --> this person won
            if (temp_array.every(function (item) {
            ...
            else if (temp_array.every(function (item) {
                if (typeof item != "undefined") {
                    return !item;
                }
                else {
    ...
    // cheks all rows
    for (var index_r = 0; index_r < default_settings.grid_dimensions.y; index_r++) {
        // for each row
        for (var index_c = 0; index_c < default_settings.grid_dimensions.x; index_c++) {
            // for each column
            var temp_array = [];
            for (var index_b = index_c; index_b < default_settings.game_settings.ponts_to_win + index_c &&
                index_b < default_settings.grid_dimensions.x; index_b++) {
    ...
    for (var i2 = 0; i2 <= default_settings.grid_dimensions.y - default_settings.game_settings.ponts_to_win; i2++) {
        for (var i1 = 0; i1 <= default_settings.grid_dimensions.x - default_settings.game_settings.ponts_to_win; i1++) {
            var temp_array = [];
            for (var i3 = i1, i4 = i2 + default_settings.game_settings.ponts_to_win; i3 < default_settings.game_settings.ponts_to_win + i1 && i4 > i2; i3++, i4--) {
    ...
    for (var next_y = 0; next_y <= default_settings.grid_dimensions.y - default_settings.game_settings.ponts_to_win; next_y++) {
        for (var next_x = 0; next_x <=
            default_settings.grid_dimensions.x - default_settings.game_settings.ponts_to_win; next_x++) {
            var temp_array = [];
            /* debugging
            context.clearRect(0, 0, canvas.width, canvas.height);
            start_game();
            debugging */
            for (var this_x = next_x + default_settings.game_settings.ponts_to_win, this_y = next_y + default_settings.game_settings.ponts_to_win; this_y > next_y && this_x > next_x; this_y--, this_x--) {
    ...
    var first_value = filled_boxes[0][0];
    var all_boxes_checker = true;
    if (typeof first_value != "undefined") {
        filled_boxes.forEach(function (row) {
